...
hashtable **hashtable_init(int L, int dataset_size)
{
    ...
    for (i = 0 ; i < L ; i++)
    {
        ...
        hashtable_array[i]->size = dataset_size/16;
...
// Auxiliary function that crosses the hashtable to return the
// index-th curve  (index has range 1-n)
node *hashtable_crossing(hashtable *ht, int index)
{
	int i;
	node *tmp;
