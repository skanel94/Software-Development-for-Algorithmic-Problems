...
void PAM(segment **confs_array, int *centroids, cluster **clusters, int k_cluster, double **distances_array)
{
	int i, j, z, min_index;	
	double min;
	double *distances;
	...
	for (i = 0 ; i < k_cluster ; i++)
	{
		...
		for (j = 0 ; j < clusters[i]->cluster_size ; j++)
		{
			...
			for (z = 0 ; z < clusters[i]->cluster_size ; z++)
				distances[j] += distances_array[clusters[i]->segs_in_cluster[j]][clusters[i]->segs_in_cluster[z]];
		...
		centroids[i] = clusters[i]->segs_in_cluster[min_index];
