...
double cRMSD(double **coord1, double **coord2, int N, int metric_function)
{
	int i, j;
	int *ipiv;
	...
	for (i = 0 ; i < N ; i++)
	{
		coord1_translate[i] = (double*) malloc (2 * sizeof(double));
		...
		coord2_translate[i] = (double*) malloc (2 * sizeof(double));
	...
	c = (double*) malloc(2 * 2 * sizeof(double));
	...
	tmp1_matrix = create_1D_array(coord1_transpose, 2, N);
	tmp2_matrix = create_1D_array(coord2_translate, N, 2);
	
	cblas_dgemm(CblasRowMajor, CblasNoTrans , CblasNoTrans, 2, 2, N, 1.0, tmp1_matrix, 
				N, tmp2_matrix, 2, 0.0, c, 2);

	free(tmp1_matrix);
	free(tmp2_matrix);
	
	// SVD
	singular = LAPACKE_malloc(2 * sizeof(double));
	...
	V_transpose = LAPACKE_malloc(2 * 2 * sizeof(double));	
	...
	U = LAPACKE_malloc(2 * 2 * sizeof(double));
	...
	LAPACKE_dgesvd(LAPACK_ROW_MAJOR, 'A', 'A', 
					2, 2, c, 2, singular, U, 
					2, V_transpose, 2, superb);

	// If σ2 is negative , we have to stop the procedure
	if (singular[1] < 0 )
	{
	...
	Q = LAPACKE_malloc(2 * 2 * sizeof(double));	
	...
	cblas_dgemm(CblasRowMajor, CblasNoTrans , CblasNoTrans,
				2, 2, 2, 1.0, U, 
				2, V_transpose, 2, 0.0, Q, 2);


	// Calculate and check determinant of Q matrix
	ipiv = (int*) malloc(2 * sizeof(int));
	...
	if(LAPACKE_dgetrf(LAPACK_ROW_MAJOR, 2, 2, Q, 2, ipiv) != 0)
	{
	...
	tmp_matrix = LAPACKE_malloc(N * 2 * sizeof(double));
	...
	// tmp_matrix = X * Q
	tmp1_matrix = create_1D_array(coord1_translate, N, 2);
	
	cblas_dgemm(CblasRowMajor, CblasNoTrans , CblasNoTrans,
				N, 2, 2, 1.0, tmp1_matrix, 
				2, Q, 2, 0.0, tmp_matrix, 2);
	...
	{			
		for (i = 0 ; i < N ; i++)
			for (j = 0 ; j < 2 ; j++)
				coord1_translate[i][j] = tmp1_matrix[i*2+j];
	...
	else
	{
		...
		distance = DTW_distance(coord1_translate, coord2_translate, N);
	}
...
double **transpose(double **coord, int N)
{
	...
	trans = (double**) malloc(2 * sizeof(double*));
...
double **find_centroids(double **coord1, double **coord2, int N)
{
	...
	for (i = 0 ; i < 2 ; i++)
	{
		...
		// Initialize centroids to x = 0, y = 0
		for (j = 0 ; j < 2 ; j++)
			centroids[i][j] = 0.0;
	}
