...
cluster **Assignment_1(segment **confs_array, int k_cluster, int *centroids, int num_conf, double **distances_array)
{
	int i, j, ID;
	...
	for(i = 0; i < k_cluster ; i++)
	{
		...
		clusters[i]->segs_in_cluster = (int*) malloc(sizeof(int));
		
		if (clusters[i]->segs_in_cluster == NULL)
		{
			printf("Malloc: memory allocation error!\n");
			exit(3);
		}
		
		clusters[i]->segs_in_cluster[0] = centroids[i];
	}
	...
	{		
		...
		{	
			...
			clusters[ID]->segs_in_cluster = (int*) realloc(clusters[ID]->segs_in_cluster, clusters[ID]->cluster_size * sizeof(int));
			
			if (clusters[ID]->segs_in_cluster == NULL)
			{
				printf("Malloc: memory allocation error!\n");
				exit(3);
			}
			
			clusters[ID]->segs_in_cluster[clusters[ID]->cluster_size-1] = i;
		}
	}	
	return clusters;
}
