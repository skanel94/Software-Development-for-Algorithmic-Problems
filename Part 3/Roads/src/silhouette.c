...
double silhouette(segment **confs_array, int *centroids, cluster **clusters, int k_cluster, double **distances_array)
{
	int i, j, k, index;
	...
	for (i = 0 ; i < k_cluster ; i++)
	{
		...
		for (j = 0 ; j < clusters[i]->cluster_size ; j++)
		{
			...
			sum = distances_array[centroids[i]][clusters[i]->segs_in_cluster[j]];
            			
			// Calculate the sum distance among this object and every other object in this cluster			
			for (k = 0 ; k < clusters[i]->cluster_size ; k++)
				sum += distances_array[clusters[i]->segs_in_cluster[j]][clusters[i]->segs_in_cluster[k]];
			...
			for (k = 0 ; k < k_cluster ; k++)
				if (k != i)
				{	
					sum = distances_array[clusters[i]->segs_in_cluster[j]][centroids[k]];
			...
			for (k = 0 ; k < clusters[index]->cluster_size ; k++)
				sum += distances_array[clusters[i]->segs_in_cluster[j]][clusters[index]->segs_in_cluster[k]];
		...
		total_s[i] = (double) s_total/clusters[i]->cluster_size;	
	}
