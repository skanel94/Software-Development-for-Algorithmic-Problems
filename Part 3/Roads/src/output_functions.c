...
void output(char *output_file, cluster **clusters, int k_cluster, double silhouette, double time)
{
  	int i, j;
	...
	fprintf(output, "clustering time:\t%.5f seconds\n", time);
	
  	for (i = 0 ; i < k_cluster ; i++)
	{
		// Sort	IDs before exporting to file	
		quickSort(clusters[i]->segs_in_cluster, 0, clusters[i]->cluster_size -1);
		
		for (j = 0 ; j < clusters[i]->cluster_size ; j++)
			fprintf(output, "%d\t", clusters[i]->segs_in_cluster[j]);
		fprintf(output, "\n");
	}
